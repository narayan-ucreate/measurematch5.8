                           ...
                           <div class="form-group custom-dropdown">
                            ...
                            <select placeholder="Choose" class="selectpicker" name="type_of_organization" id="type_of_organization">
                                ...
                                @foreach($type_of_org as $type)
                                    <option @if(isset($result[0]['buyer_profile']->type_of_organization_id) && trim($result[0]['buyer_profile']->type_of_organization_id)==$type->id)
                                            {{ 'selected="selected"' }}@endif value="{{ $type->id }}">
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            ...
                            <div class="form-group">
                                <label></label>
                                <input type="button" id="updateByr" data-id="{{ config('constants.VENDOR') }}" class="update-button" value="Update">
                            </div>
                        </form>
                    </div>
                </div><!-- /.box -->
