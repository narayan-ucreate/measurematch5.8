...
<section class="expert_buyer_login user_login_block mainwrap">
    <div class="container">
        <div class="row">
            <div class="user-login-middle-block login">


                <div class="signup_landing_page user_login_section login-main">
                <h2>Login</h2>
                    <form  role="form" method="POST" action="{{ url('/login',[],$ssl) }}">
                        ...
                        <div class="form-group{{ $errors->has('error') ? ' has-error' : '' }}">
                            ...
                            <input tabindex="1" id="email" type="text" class="form-control" placeholder="you@your-business.com" value="{{$email?? ''}}"  name="mm_email" style="text-transform:lowercase;" >
                        </div>
                       
                        <input type="hidden" id="selected_email" value="{{ (isset($email)?$email :'') }}" name="selected_email">
                        ...
                        @php $is_post_project_from_home=(strpos(URL::previous(), 'postproject/finalstep') !== false || strpos(URL::previous(), 'post-project-login-from-homepage') !== false)?'1':'0'; @endphp
                         <input id="posted_project_from_homepage" type="hidden" class="form-control" value="{{$is_post_project_from_home}}" name="posted_project_from_homepage">
                        ...
                        <div class="check-box-design">
                            ...
                            <a class="btn btn-link gilroyregular-bold-font pull-right forgot-password-link" href="{{ url('/password/reset',[],$ssl) }}">Forgot password?</a>
                        </div>
